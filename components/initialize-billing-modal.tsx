...
import { Receipt, Loader2, Calendar, ChevronDown } from "lucide-react";
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-5 pt-2">
                ...
                {sourceOfferings.length === 0 ? (
                  ...
                  sourceOfferings.map((o) => (
                    <SelectItem key={o.id} value={o.id}>
                      {o.program.code} - {o.term.name} (Sem {o.semesterNumber})
                      • {o._count?.enrollments || 0} students
                    </SelectItem>
                  ))
                )}
              </SelectContent>
              ...
              <label className="flex cursor-pointer items-center gap-3 border-b border-gray-100 px-4 py-3 hover:bg-gray-50">
                ...
                {enrollments.length > 0 && (
                  <span className="ml-auto text-xs text-gray-400">
                    {selectedStudentIds.size} of {enrollments.length} selected
                  </span>
                )}
              </label>
                ...
                {loading ? (
                  ...
                  enrollments.map((enrollment) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={selectedStudentIds.has(enrollment.student.id)}
          ...
          {/* Migration Summary */}
          {sourceOffering && targetOffering && selectedStudentIds.size > 0 && (
            <div className="rounded-lg border border-blue-100 bg-blue-50 p-3">
              <p className="text-sm text-blue-800">
                ...
                will be enrolled in{" "}
                <span className="font-semibold">
                  {targetOffering.program.code} - {targetOffering.term.name}{" "}
                  (Semester {targetOffering.semesterNumber})
                </span>{" "}
